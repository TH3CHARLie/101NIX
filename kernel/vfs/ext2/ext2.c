...
u32 init_ext2(u32 base, u8 * DEV_NAME) {
    ...
    ext2_root_inode->i_op               = &ext2_dir_inode_operations;
...
u32 ext2_create(struct inode *dir, struct dentry *dentry, u32 mode) {
    struct inode * inode;
    u32 err = 0;

#ifdef DEBUG_EXT2
    kernel_printf("%snow in ext2_create()\n", quad1);
#endif
...
struct dentry * ext2_inode_lookup(struct inode * dir, struct dentry * dentry, struct nameidata * nd) {
    ...
    for ( i = 0; i < dir->i_blocks; i++){
        ...
        while (*data != 0 && data < end) {
            ...
            if (generic_compare_filename(&qstr, &(dentry->d_name)) == 0) {
                ...
                found = 1;
#ifdef DEBUG_EXT2
                kernel_printf("        [ext2 inode lookup]: found!\n");
#endif
                break;
            }
            data += (ex_dir_entry->rec_len);
...
#ifdef DEBUG_EXT2
        kernel_printf("        [ext2 inode lookup]: found: %d\n", found);
#endif
    }
    ...
    if (!found) {
#ifdef DEBUG_EXT2
        kernel_printf("        [ext2 inode lookup]: inode not found!\n");
#endif
        return 0;
    }
