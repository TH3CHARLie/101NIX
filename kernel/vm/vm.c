      ...
      for (j = 0; j < PAGE_SIZE / sizeof(void*); j++) {
        ...
        if (pte != NULL) {
          ...
          kernel_printf("virtual addr = %x, physical addr = %x\n",
                        (unsigned int)virtual_addr, (unsigned int)pte);
        }
      }
    }
  }
...
void memory_pool_delete(task_struct* pcb) {
  ...
  for (i = 0; i < 9; i++) {
    ...
    while (memory_block) {
      ...
      //     "physical_addr:%x\n",
      //     memory_block->base_virtual_addr, memory_block->base_physical_addr);
  ...
  if (physical_addr != NULL) {
    ...
    while (curr_memory_block) {
      ...
      if (physical_addr >= start_addr && physical_addr < end_addr) {
        virtual_addr = physical_addr - curr_memory_block->base_physical_addr +
